...
class CommandExecutor(private val context: Context) {
    ...
    fun execute(command: ParsedCommand, originalText: String): CommandResult = try {
        when (command.type) {
            ...
            CommandType.WIFI_ON, CommandType.WIFI_OFF -> wifiPanel()
            CommandType.BLUETOOTH_ON, CommandType.BLUETOOTH_OFF -> btPanel()
            CommandType.NOVA_STOP -> CommandResult(true, "NOVA going to sleep. Say your wake word to activate me again.")
            CommandType.NOVA_HELP -> CommandResult(true, "You can say: call a contact, open an app, volume up or down, flashlight on or off, take a screenshot, go home, go back, or volume mute.")
            CommandType.NOVA_STATUS -> CommandResult(true, "I am running fully offline. All systems operational.")
            CommandType.UNKNOWN -> CommandResult(false, "I did not understand that. Try saying help for available commands.")
            else -> CommandResult(false, "Command not implemented yet.")
        }
    } catch (e: Exception) { CommandResult(false, "Something went wrong. Please try again.") }

    private fun makeCall(name: String): CommandResult {
        if (name.isBlank()) return CommandResult(false, "Who should I call?")
        return try {
            context.startActivity(Intent(Intent.ACTION_CALL, Uri.parse("tel:$name")).apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK })
            CommandResult(true, "Calling $name")
        } catch (_: Exception) {
            context.startActivity(Intent(Intent.ACTION_DIAL, Uri.parse("tel:$name")).apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK })
    ...
    private fun openApp(name: String): CommandResult {
        ...
        return if (match != null) {
            pm.getLaunchIntentForPackage(match.packageName)?.let {
                it.flags = Intent.FLAG_ACTIVITY_NEW_TASK; context.startActivity(it)
                CommandResult(true, "Opening ${pm.getApplicationLabel(match)}")
            } ?: CommandResult(false, "Cannot open $name")
    ...
    private fun vol(dir: Int): CommandResult {
        ...
        return CommandResult(true, when(dir) { AudioManager.ADJUST_RAISE -> "Volume up"; AudioManager.ADJUST_LOWER -> "Volume down"; else -> "Muted" })
    }
    ...
    } catch (_: Exception) { CommandResult(false, "Could not control flashlight") }
    ...
    private fun brightness(up: Boolean): CommandResult = try {
        if (Settings.System.canWrite(context)) {
            ...
            Settings.System.putInt(context.contentResolver, Settings.System.SCREEN_BRIGHTNESS, if (up) minOf(255, cur+40) else maxOf(10, cur-40))
            CommandResult(true, if (up) "Brightness increased" else "Brightness decreased")
        } else {
            context.startActivity(Intent(Settings.ACTION_MANAGE_WRITE_SETTINGS, Uri.parse("package:${context.packageName}")).apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK })
            CommandResult(false, "Please grant write settings permission")
        }
    } catch (_: Exception) { CommandResult(false, "Could not change brightness") }

    private fun wifiPanel(): CommandResult {
        val i = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) Intent(Settings.Panel.ACTION_WIFI) else Intent(Settings.ACTION_WIFI_SETTINGS)
        context.startActivity(i.apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK })
        return CommandResult(true, "Opening WiFi settings")
    }

    private fun btPanel(): CommandResult {
        val i = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) Intent(Settings.Panel.ACTION_BLUETOOTH) else Intent(Settings.ACTION_BLUETOOTH_SETTINGS)
        context.startActivity(i.apply { flags = Intent.FLAG_ACTIVITY_NEW_TASK })
    ...
    private fun accessibility(action: String): CommandResult {
        ...
        return if (svc != null) { svc.performNamedAction(action); CommandResult(true, "Done") }
        else CommandResult(false, "Enable Accessibility Service in Settings for this command")
    }
}